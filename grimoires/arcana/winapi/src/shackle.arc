...
shackle type WideText = Vec<u16>
shackle type ResultUnit = Result<(), String>
shackle type ResultModule = Result<HMODULE, String>
shackle type ResultString = Result<String, String>
shackle type ResultCatalog = Result<SystemFontCatalogState, String>
shackle type CatalogStateRef = &'static SystemFontCatalogState
shackle type CatalogEntryRef = &'static SystemFontEntry
shackle type ResultCatalogRef = Result<CatalogStateRef, String>
shackle type ResultCatalogEntryRef = Result<CatalogEntryRef, String>
...
shackle fn window_class_name() -> WideText:
    "ArcanaHiddenWindow\0".encode_utf16().collect::<Vec<u16>>()

shackle fn current_module_handle_for_address(address: LPCVOID) -> ResultModule:
...
shackle fn module_path_text(module: HMODULE) -> ResultString:
...
shackle fn register_hidden_window_class() -> ResultUnit:
    HIDDEN_WINDOW_CLASS
        .get_or_init(|| {
...
shackle fn build_system_font_catalog() -> ResultCatalog:
...
shackle fn catalog_ref(handle: u64) -> ResultCatalogRef:
...
shackle fn catalog_entry(handle: u64, index: i64) -> ResultCatalogEntryRef:
    if index < 0 {
...
shackle fn destroy_catalog_handle(handle: u64):
...
shackle thunk hidden_window_proc(hwnd: HWND, message: UINT, wparam: WPARAM, lparam: LPARAM) -> LRESULT = win32.hidden_window_proc:
    ...
    if message == WM_APP + 1 && !instance.is_null() {
        let args = [
            binding_opaque(hwnd as usize as u64),
            binding_int(message as i64),
            binding_int(wparam as i64),
            binding_int(lparam as i64),
        ];
        match unsafe { invoke_callback_int_result(&mut *instance, "window_proc", &args) } {
            Ok(result) => {
                unsafe {
                    (*instance).package_state.last_callback_code = result;
                }
...
shackle fn foundation_current_module_impl() -> arcana_winapi.types.ModuleHandle = foundation.current_module:
    Ok(binding_opaque(
        current_module_handle_for_address(hidden_window_proc as usize as LPCVOID)? as usize as u64
    ))

shackle fn foundation_module_is_null_impl(read module: arcana_winapi.types.ModuleHandle) -> Bool = foundation.module_is_null:
    Ok(binding_bool(module == 0))

shackle fn foundation_module_path_impl(read module: arcana_winapi.types.ModuleHandle) -> Str = foundation.module_path:
...
shackle fn fonts_system_font_catalog_impl() -> arcana_winapi.types.SystemFontCatalog = fonts.system_font_catalog:
    let catalog = Box::new(build_system_font_catalog()?);
    Ok(binding_opaque(Box::into_raw(catalog) as usize as u64))

shackle fn fonts_catalog_count_impl(read catalog: arcana_winapi.types.SystemFontCatalog) -> Int = fonts.catalog_count:
    Ok(binding_int(catalog_ref(catalog)?.entries.len() as i64))

shackle fn fonts_catalog_family_name_impl(read catalog: arcana_winapi.types.SystemFontCatalog, index: Int) -> Str = fonts.catalog_family_name:
    Ok(binding_owned_str(catalog_entry(catalog, index)?.family_name.clone()))

shackle fn fonts_catalog_face_name_impl(read catalog: arcana_winapi.types.SystemFontCatalog, index: Int) -> Str = fonts.catalog_face_name:
    Ok(binding_owned_str(catalog_entry(catalog, index)?.face_name.clone()))

shackle fn fonts_catalog_full_name_impl(read catalog: arcana_winapi.types.SystemFontCatalog, index: Int) -> Str = fonts.catalog_full_name:
    Ok(binding_owned_str(catalog_entry(catalog, index)?.full_name.clone()))

shackle fn fonts_catalog_postscript_name_impl(read catalog: arcana_winapi.types.SystemFontCatalog, index: Int) -> Str = fonts.catalog_postscript_name:
    Ok(binding_owned_str(catalog_entry(catalog, index)?.postscript_name.clone()))

shackle fn fonts_catalog_path_impl(read catalog: arcana_winapi.types.SystemFontCatalog, index: Int) -> Str = fonts.catalog_path:
    Ok(binding_owned_str(catalog_entry(catalog, index)?.path.clone()))

shackle fn fonts_catalog_destroy_impl(take catalog: arcana_winapi.types.SystemFontCatalog) = fonts.catalog_destroy:
    destroy_catalog_handle(catalog);
    Ok(binding_unit())

shackle fn windows_create_hidden_window_impl() -> arcana_winapi.types.HiddenWindow = windows.create_hidden_window:
...
shackle fn windows_post_ping_impl(read window: arcana_winapi.types.HiddenWindow, code: Int) = windows.post_ping:
...
shackle fn windows_destroy_hidden_window_impl(take window: arcana_winapi.types.HiddenWindow) = windows.destroy_hidden_window:
    let hwnd = window as usize as HWND;
