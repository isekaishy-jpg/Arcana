...
- structured qualified phrase, unary/binary operator, `>> await`, and `weave`/`split` expression parsing over the imported operator/async corpus, with opaque fallback still covering the remaining hard cases

## Next Milestones

1. Replace the remaining opaque expression/phrase cases with real parsing for field/index access, collection forms, chain phrases, memory phrases, and other frozen-language leftovers.
