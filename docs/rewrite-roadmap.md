...
- frontend semantic validation now includes conservative body-level value resolution for locals, chain steps, memory constructors, rollup handlers, and package/module-qualified value roots over the imported selfhost corpus

## Next Milestones

1. Replace the remaining ambiguous bracket edge cases and raw opaque-expression fallbacks with fully structured parsing.
