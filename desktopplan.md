...
## Summary
- Replace the current `arcana_desktop` scaffold with a real desktop facade that reaches near-full winit-style parity on a Windows-first, platform-agnostic Arcana API.
- Keep the facade Arcana-idiomatic: a statically-dispatched app runner with explicit session/window state, no dynamic dispatch, no hidden globals.
...
- Build the real `arcana_desktop` facade on top of that substrate.
...
- Substrate and backend
  ...
  - Preserve the current simple per-window pump as a compatibility path, but route all new broad-parity facade work through the session model.
- Desktop grimoire architecture
  - Rebuild `arcana_desktop` around the session runner instead of thin wrapper modules.
...
- Facade acceptance tests
  ...
  - Completion bar: at least one native Windows sample app runs entirely through `arcana_desktop` without direct app-shell use of `std.window` / `std.events` / `std.input`.
...
- The desktop facade owns the app shell only; graphics/text convenience stays in the separate grimoires.
