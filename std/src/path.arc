...
export fn cwd() -> Str:
    return std.kernel.host.path_cwd :: :: call

export fn join(a: Str, b: Str) -> Str:
    return std.kernel.host.path_join :: a, b :: call

export fn normalize(path: Str) -> Str:
    return std.kernel.host.path_normalize :: path :: call

export fn parent(path: Str) -> Str:
    return std.kernel.host.path_parent :: path :: call

export fn file_name(path: Str) -> Str:
    return std.kernel.host.path_file_name :: path :: call

export fn ext(path: Str) -> Str:
    return std.kernel.host.path_ext :: path :: call

export fn is_absolute(path: Str) -> Bool:
    return std.kernel.host.path_is_absolute :: path :: call

export fn stem(path: Str) -> Result[Str, Str]:
    let pair = std.kernel.host.path_stem_try :: path :: call
    if pair.0:
...
export fn with_ext(path: Str, ext: Str) -> Str:
    return std.kernel.host.path_with_ext :: path, ext :: call

export fn relative_to(path: Str, base: Str) -> Result[Str, Str]:
    let pair = std.kernel.host.path_relative_to_try :: path, base :: call
    if pair.0:
        return Result.Ok[Str, Str] :: pair.1 :: call
    return Result.Err[Str, Str] :: (std.kernel.host.last_error_take :: :: call) :: call
